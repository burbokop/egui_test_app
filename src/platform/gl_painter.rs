...
impl Painter {
    pub fn new(canvas: &mut inkview::Canvas<'static>, scale: f32, shader_ver: ShaderVersion) -> Painter {
        unsafe {
            /* 
...
*/
    ...
    pub fn update_screen_rect(&mut self, size: (usize, usize)) {
    ...
    fn upload_egui_texture(&mut self, texture_version: u64, texture: &AlphaImage) {
        if self.egui_texture_version == Some(texture_version) {
            return; // No change
        }
        ...
        unsafe {
            ...
            self.egui_texture_version = Some(texture_version);
        }
    }
    ...
    pub fn paint_jobs(
        ...
        self.upload_egui_texture(texture_version, texture);
